...
export default function App() {
  const [tab,setTab]=useState("today"),[days,setDays]=useState(PRELOADED_DAYS),[tasks,setTasks]=useState(PRELOADED_TASKS),[rails,setRails]=useState(defaultRails),[loaded,setLoaded]=useState(false);
  ...
  const nav=[{id:"today",label:"Today",icon:ic.today},{id:"week",label:"Week",icon:ic.week},{id:"exec",label:"Execute",icon:ic.exec},{id:"rails",label:"Rails",icon:ic.rails}];
  return (
    <div style={{maxWidth:600,margin:"0 auto",minHeight:"100vh",background:M.bg,fontFamily:font,position:"relative",display:"flex",flexDirection:"column",overflow:"hidden",boxSizing:"border-box"}}>
      {tab==="today"&&<button onClick={()=>setTab("settings")} style={{position:"absolute",top:20,right:18,zIndex:45,width:36,height:36,borderRadius:18,background:"transparent",border:"none",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}><Ic d={ic.settings} s={20} c={M.onSurfaceV}/></button>}
      ...
      <nav style={{position:"fixed",bottom:0,left:"50%",transform:"translateX(-50%)",width:"100%",maxWidth:600,background:M.surfaceC,padding:"0 8px 8px",display:"flex",justifyContent:"space-around",zIndex:40,boxShadow:"0 -1px 3px rgba(0,0,0,.08)"}}>
        {nav.map(n=>{const a=tab===n.id;return(
          <button key={n.id} onClick={()=>setTab(n.id)} style={{display:"flex",flexDirection:"column",alignItems:"center",gap:4,background:"none",border:"none",cursor:"pointer",padding:"12px 16px 8px"}}>
            <div style={{padding:"4px 20px",borderRadius:16,background:a?M.primaryC:"transparent",transition:"all .2s"}}><Ic d={n.icon} s={24} c={a?M.onPrimaryC:M.onSurfaceV}/></div>
            <span style={{...T.labelS,color:a?M.onSurface:M.onSurfaceV,fontWeight:a?700:500}}>{n.label}</span>
          </button>
        );})}
      </nav>
